...
void ForwardDynamics (
	...
	for (i = 1; i < model.mBodies.size(); i++) {
		...
		if (f_ext != NULL && (*f_ext)[i] != SpatialForce (Vector3d (0., 0., 0.), Vector3d (0., 0., 0.))) {
			LOG << "External force (" << i << ") = " << model.X_base[i].apply((*f_ext)[i]) << std::endl;
			model.pA[i] -= model.X_base[i].apply ((*f_ext)[i]);
		}
	}
...
// ClearLogOutput();
	...
	for (i = model.mBodies.size() - 1; i > 0; i--) {
		...
		if (model.mJoints[i].mDoFCount == 3) {
			model.multdof3_U[i] = model.IA[i].toMatrix() * model.multdof3_S[i];
#ifdef EIGEN_CORE_H
			...
			model.multdof3_u[i] = tau_temp - model.multdof3_S[i].transpose() * model.pA[i].toVector();

//			LOG << "multdof3_u[" << i << "] = " << model.multdof3_u[i].transpose() << std::endl;
			unsigned int lambda = model.lambda[i];
			if (lambda != 0) {
				SpatialInertia Ia = model.IA[i] - SpatialInertia::fromMatrix(model.multdof3_U[i] * model.multdof3_Dinv[i] * model.multdof3_U[i].transpose());
				SpatialForce pa = model.pA[i] + Ia * model.c[i] + SpatialForce::fromVector(model.multdof3_U[i] * model.multdof3_Dinv[i] * model.multdof3_u[i]);

				model.IA[lambda] += model.X_lambda[i].applyInverse(Ia);
				model.pA[lambda] += model.X_lambda[i].applyInverse(pa);
				LOG << "pA[" << lambda << "] = " << model.pA[lambda] << std::endl;
			}
		} else {
...
//			LOG << "u[" << i << "] = " << model.u[i] << std::endl;
			...
			unsigned int lambda = model.lambda[i];
			if (lambda != 0) {
				SpatialInertia Ia = model.IA[i] - SpatialInertia::fromMatrix(model.U[i].toVector() * (model.U[i].toVector() / model.d[i]).transpose());
				SpatialForce pa = model.pA[i] + Ia * model.c[i] + model.U[i] * model.u[i] / model.d[i];
...
//	ClearLogOutput();
	...
	for (i = 1; i < model.mBodies.size(); i++) {
		...
		if (model.mJoints[i].mDoFCount == 3) {
			Vector3d qdd_temp = model.multdof3_Dinv[i] * (model.multdof3_u[i] - model.multdof3_U[i].transpose() * model.a[i].toVector());
...
void InverseDynamics (
	...
	for (i = 1; i < model.mBodies.size(); i++) {
		...
		if (lambda == 0) {
			...
			if (model.mJoints[i].mDoFCount == 3) {
				model.a[i] = model.a[i] + SpatialMotion::fromVector(model.multdof3_S[i] * Vector3d (QDDot[q_index], QDDot[q_index + 1], QDDot[q_index + 2]));
			} else {
				model.a[i] = model.a[i] + model.S[i] * QDDot[q_index];
			}	
		...
		model.f[i] = model.mBodies[i].mSpatialInertia * model.a[i] + model.v[i].crossf(model.mBodies[i].mSpatialInertia * model.v[i]);
		if (f_ext != NULL && (*f_ext)[i] != SpatialForce (Vector3d (0., 0., 0.), Vector3d (0., 0., 0.))) {
			model.f[i] -= model.X_base[i].apply((*f_ext)[i]);
		}
	}
	...
	for (i = model.mBodies.size() - 1; i > 0; i--) {
		...
		if (model.mJoints[i].mDoFCount == 3) {
			Vector3d tau_temp = model.multdof3_S[i].transpose() * model.f[i].toVector();
		...
		if (lambda != 0) {
			model.f[lambda] = model.f[lambda] + model.X_lambda[i].applyInverse(model.f[i]);
		}
	}
}
		...
		if (model.mJoints[i].mDoFCount == 3) {
			...
			while (model.lambda[j] != 0) {
				...
				if (model.mJoints[j].mDoFCount == 3) {
					...
					Vector3d H_temp2 = F_63.transpose() * (model.S[j].toVector());
			...
			SpatialForce F = model.Ic[i] * model.S[i];
